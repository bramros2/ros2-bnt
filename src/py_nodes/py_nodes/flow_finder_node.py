...
class BlobDetector(Node):
    ...
    def __init__(self, thr_min, thr_max, blur=15, blob_params=None, detection_window=None):
        ...
        #self.image_pub = rclpy.Publisher("/blob/image_blob",Image,queue_size=1)
        self.image_pub = self.create_publisher(Image, "image/flow", 1)

        #self.mask_pub = rclpy.Publisher("/blob/image_mask",Image,queue_size=1)
        self.mask_pub = self.create_publisher(Image, "image/mask", 1) 

        print (">> Publishing position to topic point_blob")
 
        #self.blob_pub  = rclpy.Publisher("/blob/point_blob",Point,queue_size=1)
        self.blob_pub = self.create_publisher(Point, "image/point", 1)


        self.bridge = CvBridge()

        #self.image_sub = rclpy.Subscriber("/image_raw",Image,self.callback)
